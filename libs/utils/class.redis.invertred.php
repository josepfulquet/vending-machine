 ...
 * CLASS RedisInvertred
	Based on https://github.com/phpredis/phpredis


	Sample calls

	By default, this class closes connection after each operation and doesn't persists inmediately to disk.
	This behavior with 'options' array.

	'options' must be an array with this available options:
		- 'persist' => true						persist inmediately to disk
		- 'nodisconnect' => true			mantains connection to redis db opened
		- 'hostNumber' => hostNumber defined on config.php (default 0)
		- 'ttl' => time to life in seconds
		- 'db' => db number (default 0)

	INSERTS/UPDATES
	setKey('foo',"122334",array());																				Sets key
	setKey('foo',"122334",array("persist"=>true));												Sets keys and persist inmediately to disk
	setKey('foo',"122334",array("persist"=>true,"nodisconnect"=>true));		Sets keys and persist inmediately to disk and doesn't close connection
	setKey('foo',"122334",array("nodisconnect"));													Sets keys and doesn't close connection
	* $options array can be used in all operations
	setHash('foo',array("testing"=>"your mother","id"=>1),array());				Sets a redis hash (content is an array key/value)
	setHashKey('foo',"keyname","value",array());													Sets a key value of a redis hash

	DELETES
	deleteKey('foo',array());																							Deletes a key
	deleteHashKey('foo',"uno",array("persist"=>true));										Deletes a key from a redis hash

	GETTERS
	getRedisKey('foo',array();																						Gets a key
	getHashKey('foo',"uno",array()); 																			Gets a key from a redis hash
	getHash('foo',array());																								Gets a redis hash
	getKeys('foo:*',array());																							Gets al existing keys matching a pattern
	existsKey('foo:1',array());																						Checks if a key exists

	For custom operations and concatenations. You muts handle connection closing
